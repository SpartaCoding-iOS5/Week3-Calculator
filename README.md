...
이전의 계산기는 사용자가 `1++`를 입력하더라도 그대로 화면에 출력되고, `=`으로 계산하려고 한다면 에러가 발생하도록 구현했었다. 허나, 실제 iOS 계산기는 연산자를 재입력 하거나 중복해서 사용할 경우 나중에 입력한 연산자로 바뀌게 되어있었다.
이를 보고 같은 기능을 구현해보자 하였고, 입력된 값이 연산자인지 확인한 후 현재 레이블의 마지막 값이 연산자인지 확인하는 메소드를 만들었다. 이 메소드는 `Bool` 을 반환하여, 만약 값이 `true`일 경우 현재 레이블의 마지막 값을 삭제하고, 새로 입력된 값을 추가하는 메소드를 같이 사용해 주었다.
이렇게 하니 에러를 사전에 방지할 수 있고, 조금 더 업그레이드 된 계산기라는 생각이 들었다. 다만 한가지 걱정되는 점은 이렇게 에러를 사전에 방지함으로써 지정에러를 굳이 쓸 필요가 있나? 싶은 점이다. 즉, 에러타입을 지정해서 쓰는 것이 혹시 비효율적인건 아닐까 고민이 되었다...
사용자는 앱을 의도한 대로만 사용하지 않고, 어떤 예측 불가능한 상황이 발생할지 모르기 때문에 에러타입을 지정하는 것 자체는 문제가 되지 않는다고 생각했다. 다만, 상황에 따라 사용자 지정 에러는 조정의 여지를 줄 필요성이 있다고 결론을 내렸다.
